...
if button and uploaded_file is not None and num_mcqs and subject and complexity:
    with st.spinner("loading..."):
        try:
            ...
            # quizz = json.loads(response["quiz"])
            # for idx, mcq in enumerate(quizz, start=1):
            #     st.markdown(f"### Q{idx}: {mcq['question']}")
            #     st.write(f"- **a)** {mcq['options'][0]}")
            #     st.write(f"- **b)** {mcq['options'][1]}")
            #     st.write(f"- **c)** {mcq['options'][2]}")
            #     st.write(f"- **d)** {mcq['options'][3]}")
            #     st.markdown(f"**✔ Correct Answer:** {mcq['answer']}", unsafe_allow_html=True)

            #     st.divider()  # Adds a horizontal line between questions for better readability
